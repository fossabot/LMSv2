...
function gridMakeColorRender(record, index, column_index) {
    ...
    if (record.w2ui.changes !== undefined) {
        if (record.w2ui.changes[this.columns[column_index].field] != undefined) {
            vcolor = record.w2ui.changes[this.columns[column_index].field];
        }
    }
...
var dsGridFunctions = new function () {
    ...
    thisObjGridFunctions.modalSelect = '<div style="max-height:#hh#px; overflow-y: auto" class="row" id="gridCHGCol"><div class="col-md-12" style="font-size: 13px;">#data#</div>';
    ...
    this.showHideColumns = function (grid, target) {
        ...
        if (vcolumns.length > 10 && vwwindow > 450) {
            var vwidth = '400px';
            var vclass = 'col-md-6';
        } else {
            var vwidth = '200px';
            var vclass = 'col-md-12';            
        }
        $.each(vcolumns, function (index, value) {
            ...
            if (vCapDiv != undefined){
                info = info + '<div class="col-md-12" style="padding-bottom: 1px;1px; border-top: thin dashed black; background-color: lightgray;margin-bottom: 5px;font-size: 11px;padding-top: 1px;font-weight: bold;text-align: center">'+vCapDiv+'</div>';
        ...
        var xM = thisObjGridFunctions.modalSelect.replace('#data#', info).replace('#hh#', vhwindow);
        ...
        basicPickListOpenPopOver({
            ...
            functionOpen: function(){
                $('#gridCHGCol').cgbMakeScrollbar({autoWrapContent: false, alwaysShowScrollbar: 0, theme: 'dark'});

                $('#gridCHGCol').find('.gridColInfoSpan').each(function(){
                    var $ele = $(this);
